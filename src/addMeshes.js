...
export const matCapTest = (img, pos) => {
	let noise = tLoader.load('/noise.png')
	let image = tLoader.load(img)
	// textures.texture.wrapS = textures.texture.wrapT = RepeatWrapping
	const geometry = new PlaneGeometry(1.35, 2.4, 20, 20)
...
export const wImage = (img, pos) => {
	let noise = tLoader.load('/noise.png')
	noise.wrapS = noise.wrapT = RepeatWrapping
	let image = tLoader.load(img)
	// textures.texture.wrapS = textures.texture.wrapT = RepeatWrapping
	const geometry = new PlaneGeometry(2.4, 1.35, 20, 20)
	const MatcapSwitchMaterial = new ShaderMaterial({
		vertexShader: mVertex,
		fragmentShader: mFragment,
		transparent: true,
		...
		uniforms: {
			...
			u_Image: { value: image },
			uTime: { value: 0 },
			u_freq: { value: Math.random() * 3.0 - 1.5 },
			u_NoiseStrength: { value: 1.0 },
		},
	})
	const mesh = new Mesh(geometry, MatcapSwitchMaterial)
