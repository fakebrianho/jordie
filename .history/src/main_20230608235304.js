...
import { addIntro, matCapTest, wImage } from './addMeshes'
...
function init() {
	...
	meshes[5] = matCapTest('/4.jpg', [-4, 0, -35])
	meshes[6] = matCapTest('/20.jpg', [4, 1, -35])
	...
	meshes[11] = wImage('/17.jpg', [0.75, -1, -47])
	meshes[12] = wImage('/16.jpg', [2, 1.25, -44])
	meshes[13] = matCapTest('/21.jpg', [-1, -1, -44])
	meshes[14] = wImage('/6.jpg', [-0.5, 1.3, -44])
	meshes[15] = wImage('/18.jpg', [1, -0.76, -44])
	meshes[16] = wImage('/10.jpg', [-2, 0, -43])
	...
	window.addEventListener('click', () => {
		gsap.to('.instructions button', { opacity: 0, duration: 1 })
	})
}

function f1(_callback) {
	//
	_callback()
}
function f2() {
	...
	gsap.to('.instructions button', { opacity: 1, duration: 3, delay: 4 })
	f1(() => {
		sheet.sequence.attachAudio({ source: '/jordan.mp3' }).then(() => {
			for (let i = 0; i < Object.keys(meshes).length; i++) {
				initAnimation(i)
			}
			gsap.to('#J', { opacity: 1, duration: 2 })
			gsap.to('#O', { opacity: 1, duration: 2 })
			gsap.to('#R', { opacity: 1, duration: 2 })
			gsap.to('#D', { opacity: 1, duration: 2 })
			gsap.to('#I', { opacity: 1, duration: 2 })
			gsap.to('#E', { opacity: 1, duration: 2 })
			gsap.to('#J', { opacity: 0, duration: 2, delay: 3 })
			gsap.to('#O', { opacity: 0, duration: 2, delay: 4 })
			gsap.to('#R', { opacity: 0, duration: 2, delay: 5 })
			gsap.to('#D', { opacity: 0, duration: 2, delay: 6 })
			gsap.to('#I', { opacity: 0, duration: 2, delay: 7 })
			gsap.to('#E', { opacity: 0, duration: 2, delay: 8 })
...
function animate() {
	...
	composer.render()
	meshes[0].material.uniforms.uTime.value += 0.21
	meshes[1].material.uniforms.uTime.value += 0.15
	meshes[2].material.uniforms.uTime.value += 0.13
	meshes[3].material.uniforms.uTime.value += 0.13
	meshes[4].material.uniforms.uTime.value += 0.11
	meshes[5].material.uniforms.uTime.value += 0.12
	meshes[6].material.uniforms.uTime.value += 0.21
	meshes[7].material.uniforms.uTime.value += 0.23
	meshes[8].material.uniforms.uTime.value += 0.11
	meshes[9].material.uniforms.uTime.value += 0.11
	meshes[10].material.uniforms.uTime.value += 0.12
	meshes[11].material.uniforms.uTime.value += 0.21
	meshes[12].material.uniforms.uTime.value += 0.13
	meshes[13].material.uniforms.uTime.value += 0.12
	meshes[14].material.uniforms.uTime.value += 0.11
	meshes[15].material.uniforms.uTime.value += 0.3
	meshes[16].material.uniforms.uTime.value += 0.2
	intro.intro.material.uniforms.uTime.value += 0.055
