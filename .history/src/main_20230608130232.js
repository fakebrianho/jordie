...
function initAnimation(mesh, index) {
	...
	// )
	theatreObjects[index] = sheet.object('Torus Knot', {
		// Note that the rotation is in radians
		// (full rotation: 2 * Math.PI)
		position: types.compound({
			x: types.number(meshes[1].position.x, { range: [-2, 2] }),
			y: types.number(meshes[1].position.y, { range: [-2, 2] }),
			z: types.number(meshes[1].position.z, { range: [-2, 2] }),
		}),
	})
