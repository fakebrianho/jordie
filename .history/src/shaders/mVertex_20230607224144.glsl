...
float windStrength = 10.1; // controls the strength of the wind distortion
...
void main(){
  ...
  vec3 localPos = transformed - a_Centroid;
      float windEffect = windStrength * fbm(localPos * windFrequency + vec3(0.0, uTime * windSpeed, 0.0));
    float strength = fbm(vec3(localPos.xz, uTime));
  float frequency = fbm(vec3(localPos.xz, uTime));
  float noiseStrength = 3.55;
    vec3 noisePos = vec3(localPos.x * windFrequency + uTime * 0.4, localPos.y, localPos.z);
  // Calculate the distance between each face's centroid and a vec3(0.0, e_EffectProgress, 0.0) vector,
    ...
    localPos.y -= windStrength * sin(windFrequency * localPos.x + uTime * 0.5);
