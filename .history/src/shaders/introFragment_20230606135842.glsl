...
float pattern( in vec2 p, out vec2 q, out vec2 r, out vec2 s )
{
    ...
    q.x = fbm6( p + vec2(0.0,0.0) + (time * movement1));
    q.y = fbm6( p + vec2(5.2,1.3) - (time * movement2));

    r.x = fbm4( p + 4.0*q + vec2(1.7,9.2) + time * movement2);
    r.y = fbm4( p + 4.0*q + vec2(8.3,2.8) - time * movement1);
    
    s.x = fbm4( p + 4.0*r + vec2(13.7,9.2) + time * movement2);
    s.y = fbm4( p + 4.0*r + vec2(18.3,12.8) - time * movement1);
...
void main(void)
{
    ...
    qa = atan(fbm) * 10.5;
    float ql = 0.05 * (noise(vec2(time)) * 5.0) / abs(length(uv) - .8 + sin(qa + time));
//  float l = 0.05 / abs(length(p) - 0.8 + sin(a + time * 4.5) * 0.1);
    
    vec3 destColor = vec3(0.52, 0.2, 0.1);
    float a = atan(uv.y / uv.x) * sin(time) * 4.0;
    float l = 0.55 / length(uv2) * sin(a + time);
