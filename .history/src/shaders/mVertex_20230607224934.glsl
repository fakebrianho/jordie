...
float windStrength = .1; // controls the strength of the wind distortion
float windFrequency = 2.0; // controls how frequently the wind "blows"  
float windSpeed = 1.0; // Controls the speed of the wind effect
...
void main(){
  ...
  vec3 localPos = transformed - a_Centroid;
      float windEffect = windStrength * fbm(position * windFrequency + vec3(0.0, uTime * windSpeed, 0.0));
    float strength = fbm(vec3(position.xz, uTime));
  float frequency = fbm(vec3(position.xz, uTime));
  float noiseStrength = 3.55;
    vec3 noisePos = vec3(position.x * windFrequency + uTime * 0.4, position.y, position.z);
  // Calculate the distance between each face's centroid and a vec3(0.0, e_EffectProgress, 0.0) vector,
    ...
    localPos.z  += noise(noisePos) * windStrength * noiseStrength; 
    localPos.y -= windStrength * sin(windFrequency * position.x + uTime * 0.5);
  ...
  // Apply the wind effect by displacing the vertex along its normal
//   transformed += normal * windEffect * 30.0;
    transformed = a_Centroid + localPos;
  #include <project_vertex>

 
  vWorldPosition=(modelMatrix*vec4(localPos,1.)).xyz;
