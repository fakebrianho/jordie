...
void main() {
  vUv = uv;
  vec3 tPos = position;
  float strength = fbm(tPos.xz + uTime);
  float frequency = fbm(tPos.xz - uTime);
  tPos.y -= strength * windStrength * sin(frequencey * windFrequency * tPos.x + uTime);
  tPos.z -= strength * windStrength * sin(frequency * windFrequency * tPos.x + uTime);
