...
function init() {
	...
	meshes[3] = wImage('/7.jpg', [1, 0.25, -25])
	meshes[4] = matCapTest('/11.jpg', [-1, 0, -30])
	meshes[5] = matCapTest('/4.jpg', [1, 0, 0])
	meshes[6] = matCapTest('/20.jpg', [-2, 1, 0])
...
function initCameraAnimation() {
	const myCamera = sheet.object('camera', {
		position: types.compound({
			x: types.number(camera.position.x, { range: [-40, 5] }),
			y: types.number(camera.position.y, { range: [-40, 5] }),
			z: types.number(camera.position.z, { range: [-40, 5] }),
		}),
	})
...
function initAnimation(index) {
	...
	theatreObjects[index] = sheet.object(`${index}`, {
		position: types.compound({
			x: types.number(meshes[index].position.x, { range: [-40, 10] }),
			y: types.number(meshes[index].position.y, { range: [-40, 10] }),
			z: types.number(meshes[index].position.z, { range: [-40, 10] }),
		}),
