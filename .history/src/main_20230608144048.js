...
function init() {
	...
	meshes[0] = matCapTest('/1.jpg', [-0.5, 0, -3])
	meshes[1] = matCapTest('/2.jpg', [1, 0, -4.5])
	meshes[2] = matCapTest('/3.jpg', [0, 0, -6])
	meshes[3] = matCapTest('/4.jpg', [1, 0, 0])
	meshes[4] = matCapTest('/5.jpg', [2, 0, 0])
	meshes[5] = wImage('/6.jpg', [3, 0, 0])
	meshes[6] = wImage('/7.jpg', [-2, 1, 0])
	meshes[7] = matCapTest('/8.jpg', [-2, 1, 0])
	meshes[8] = matCapTest('/9.jpg', [-1, 1, 0])
...
function initAnimations() {
	...
	for (let i = 0; i < Object.keys(meshes).length - 1; i++) {
		initAnimation(i)
	}
}
...
function initCameraAnimation() {
	const myCamera = sheet.object('camera', {
		position: types.compound({
			x: types.number(camera.position.x, { range: [-20, 5] }),
			y: types.number(camera.position.y, { range: [-20, 5] }),
			z: types.number(camera.position.z, { range: [-20, 5] }),
		}),
	})
...
function initAnimation(index) {
	...
	theatreObjects[index] = sheet.object(`${index}`, {
		position: types.compound({
			x: types.number(meshes[index].position.x, { range: [-20, 10] }),
			y: types.number(meshes[index].position.y, { range: [-20, 10] }),
			z: types.number(meshes[index].position.z, { range: [-20, 10] }),
		}),
